...
- [x] Run both debug and `-c release`; note any difference in how often the race appears *(done on Sarah Rae's M2 with plain runs, no sanitizer needed: 10× `unsync` per build, saved as `output-unsync-debug-10runs.txt` and `output-unsync-release-10runs.txt`. The race appeared in 10/10 runs in both builds. Release loses far less per run (avg 0.84% of revenue vs. 6.59% in debug) because its loops finish in ~0.2 s and complete far fewer real purchases. Summary is in the README's Sample output section. The ThreadSanitizer crash on Calli's Mac is still worth a reboot and re-check.)*
